...
\begin{itemize}
  \item Implemented entirely in JavaScript
...
\begin{itemize}
  \item \textbf{Home Screen:}
    \begin{itemize}
        \item The Home Screen shall display the game title.
        \item The Home Screen shall provide a primary control to initiate gameplay.
        \item The Home Screen shall provide controls to access the tutorial and leaderboard.
    \end{itemize}
  \item \textbf{Tutorial:}
    \begin{itemize}
        \item The system shall provide a tutorial that explains game mechanics, scoring, and controls to the player.
    \end{itemize}
  \item \textbf{Leaderboard:}
    \begin{itemize}
        \item The system shall display a leaderboard listing high scores (e.g., top 100). If all scores cannot be viewed simultaneously, the list must be scrollable.
        \item The player's personal highest score and rank shall be clearly identifiable on the leaderboard.
    \end{itemize}
  \item \textbf{Start/Weapon Selection:}
    \begin{itemize}
        \item Upon initiating a new game, the system shall present an interface allowing the player to select their desired weapon/mallet from available options.
    \end{itemize}
  \item \textbf{Game Field:}
    \begin{itemize}
        \item The game field shall present a grid of interactive locations from which characters appear.
    \end{itemize}
  \item \textbf{Dynamic Timer and Score Display:}
    \begin{itemize}
        \item During gameplay, the interface shall continuously display the remaining game time and the player's current score.
    \end{itemize}
  \item \textbf{Pause/Resume and Exit Buttons:}
    \begin{itemize}
        \item The game interface shall provide controls for the player to pause and resume gameplay, and to exit or end the current game session.
    \end{itemize}
  \item \textbf{Mute Button:}
    \begin{itemize}
        \item The game interface shall include a control to mute and unmute game audio.
    \end{itemize}
  \item \textbf{Game Over Screen:}
    \begin{itemize}
        \item Upon game completion, an interface (e.g., "Game Over" screen) shall display the player's score for the completed session and their personal best score.
        \item The "Game Over" interface shall provide controls for the player to return to the Home Screen and to start a new game.
    \end{itemize}
\end{itemize}

Sketches and mock-ups will be supplied separately. For more detailed preliminary UI design concepts and layout ideas, please refer to Appendix~\ref{app:ui_design_concepts}.
...
\begin{enumerate}
  ...
  \item Trustee character (appearing approximately once per game) triggers a brief explosion animation ($\approx$ 0.25 s) and awards +20 points.
\end{enumerate}

\subsection{Functional Requirements}
\begin{itemize}
  \item \textbf{REQ-1.1}: Characters appear at uniformly random intervals of 0.5–1.5 s.
  \item \textbf{REQ-1.2}: Trustee character must appear randomly with approximately 5\% probability. Trustee must trigger a special explosion animation visibly overlaying the screen for $\approx$ 0.25 s and play an accompanying scream sound effect.
...
\chapter{Preliminary UI Design Concepts and Layout Notes}
\label{app:ui_design_concepts}

This appendix contains preliminary design concepts, layout ideas, and specific visual notes for the user interface of \product. These descriptions are intended to supplement the formal UI requirements in Section 3.1 and serve as a starting point for detailed UI design and mock-up creation. They are not formal requirements themselves but represent current thinking and suggestions.

\section*{Home Screen Design Concepts}
\begin{itemize}
    \item The game title, ``Whack A Prof!'', could be prominently displayed, perhaps at the top center of the screen.
    \item For primary navigation, a ``Start'' button might be positioned in the center of the screen.
    \item ``Tutorial'' and ``Leaderboard'' access buttons could be grouped logically, for instance, below the ``Start'' button.
    \item Thematic imagery, such as depictions of the professor characters, could be considered for the sides of the screen to enhance visual appeal.
\end{itemize}

\section*{Tutorial Presentation Ideas}
\begin{itemize}
    \item The tutorial explaining game mechanics could be presented using either video or a sequence of static images. The final choice would depend on what the graphics/design resources determine to be most effective.
\end{itemize}

\section*{Leaderboard Display Concepts}
\begin{itemize}
    \item The leaderboard could display a list of the top 100 scores.
    \item The player's own highest score might be highlighted or positioned at the very top of the displayed list for easy visibility, along with their rank.
\end{itemize}

\section*{Weapon Selection Interface Concepts}
\begin{itemize}
    \item When the player initiates a new game, an interface could display a variety of mallet weapon visuals in the center of the screen for selection. The specific designs of these mallets are to be determined by the graphics/design team.
\end{itemize}

\section*{Game Field Layout Concept}
\begin{itemize}
    \item A potential layout for the game field involves a 4x4 grid of 16 holes, arranged to spread across the available screen width.
    \item During gameplay, professor characters would then appear randomly from these holes.
\end{itemize}

\section*{In-Game Displays Layout Ideas}
\begin{itemize}
    \item The dynamic game timer could be positioned at the top-left of the screen.
    \item The player's current score display could be placed near the timer, for example, slightly to its right.
\end{itemize}

\section*{In-Game Controls Layout Ideas}
\begin{itemize}
    \item A Pause/Resume icon button might be located at the top-right of the screen.
    \item An Exit/End game icon button could be positioned adjacent to the Pause/Resume button (e.g., slightly to its right).
    \item A toggle Mute button could be placed near these controls, for instance, slightly to the left of the Pause/Resume button.
\end{itemize}

\section*{Game Over Screen Concepts}
\begin{itemize}
    \item Upon game completion, a distinct visual element, such as a square box, could appear in the center of the screen.
    \item This element would display both the player's score for the just-completed session and their overall best score.
    \item Below the score display, ``Home'' and ``Play Again'' buttons would allow the player to navigate accordingly.
\end{itemize}
